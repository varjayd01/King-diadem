...
def _get_time_context() -> str:
    """เวลาปัจจุบัน Bangkok สำหรับ inject เข้า prompt"""
    try:
        ...
        return (f"[เวลาปัจจุบัน: {now.strftime('%H:%M')} น. "
                f"วัน{days[now.weekday()]}ที่ {now.strftime('%d/%m/%Y')} "
                f" timezone: Bangkok UTC+7]")
    except:
        return ""
